...
def make_feedforward_nn(x_placeholder, end_h):
    W1 = tf.get_variable("W1", shape=[89, 512], initializer=tf.contrib.layers.xavier_initializer())
...
def main():
    ...
    dataset = np.loadtxt("test", delimiter=",")
    x_test_2016 = dataset[:,0:89]
    y_test_2016 = dataset[:,89].reshape(-1,1)
    ...
    dataset = np.loadtxt("test", delimiter=",")
    x_test_2017 = dataset[:,0:89]
    y_test_2017 = dataset[:,89].reshape(-1,1)
    y_test_2017 = y_test_2017-1
    #y_test_2017 = to_categorical(y_test_2017)
    #print(x_test[20])

    dataset = np.loadtxt("dev.shuffle", delimiter=",")
    x_valid = dataset[:,0:89]
    y_valid = dataset[:,89].reshape(-1,1)
    ...
    resampleFile()
    dataset = np.loadtxt("train.revised", delimiter=",")
    x_train = dataset[:,0:89]
    y_train = dataset[:,89].reshape(-1,1)
    ...
    #saver.restore(tf_session, "baselinemodel_at_epoch"+str(12)+".ckpt")
    ...
    with gpf.defer_build():
        kernel = gpf.kernels.Matern32(num_h)
    ...
    SEED = 448
    np.random.seed(SEED)
    train_unique = x_train
    shuffle = np.arange(len(train_unique))
    np.random.shuffle(shuffle)    
    x_train_unique_shuffle = train_unique[shuffle]


    gp_model.feature.Z.assign(x_train, tf_session)
    gp_model.kern.lengthscales.assign(suggest_sensible_lengthscale(phs, x_train_unique_shuffle[:len(y_train), :], h, tf_session) 
    ...
    for i in range(20): #100 epochs
        if i>0:
            ...
            #print(outputs_evald)
            #print(getMax(y_valid))
            #print("Result from the previous epoch on dev:")
            #compute_scores(y_valid, outputs_evald)
            #print(y_valid)
            #print(mean_absolute_error((y_valid), (outputs_evald)))


            if mean_absolute_error((y_valid), (outputs_evald)) < dev_max:
                dev_max = mean_absolute_error((y_valid), (outputs_evald))
                ffd = gp_model.feeds or {}
                ffd.update({phs.keep_prob: 1.0, phs.data: x_test_2016,
                           phs.label: y_test_2016})
                accuracy_evald, log_like_evald, outputs_evald = tf_session.run([avg_acc, avg_ll, outputs], feed_dict=ffd)
                ...
                test_max = (mean_absolute_error((y_test_2016), (outputs_evald)))
                output_max = outputs_evald




                ffd = gp_model.feeds or {}
                ffd.update({phs.keep_prob: 1.0, phs.data: x_test_2017,
                ...
                #outputs_evald = outputs_evald.reshape(-1, 1)
                #print(outputs_evald)
                #print("Result from the previous epoch on test adaptation:")
                #compute_scores(y_test_2017, outputs_evald)
                test_adaptation_max = (mean_absolute_error((y_test_2017), (outputs_evald)))
