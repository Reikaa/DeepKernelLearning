...
def make_small_mnist_nn(x_placeholder, keep_prob_placeholder, end_h=50):
    
    with tf.name_scope("small_convnet"):
        with tf.name_scope("layer1"):
            W1 = tf.get_variable("W1", initializer=create_weight([72, 512]))
            b1 = tf.get_variable("b1", initializer=create_weight([512]))
            h1 = tf.nn.relu(tf.matmul(x_placeholder, W1) + b1)

            #h1_drop = tf.nn.dropout(h1, keep_prob_placeholder)
        with tf.name_scope("layer2"):
            W2 = tf.get_variable("W2", initializer=create_weight([512, 512]))
            b2 = tf.get_variable("b2", initializer=create_weight([512]))

            h2 = tf.nn.relu(tf.matmul(h1, W2) + b2)

            #h2_drop = tf.nn.dropout(h2, keep_prob_placeholder)

        with tf.name_scope("layer3"):
            W3 = tf.get_variable("W3", initializer=create_weight([512, end_h]))
            b3 = tf.get_variable("b3", initializer=create_bias([end_h]))
            h4 = tf.matmul(h2, W3) + b3
...
def suggest_good_intial_inducing_points(phs: DataPlaceholders, x_data, h, tf_session, num_inducing):
    h_data = tf_session.run(h, feed_dict={phs.ximage_flat: x_data, phs.keep_prob: 1.0})
...
def standardize_data(X_train, X_test, X_valid):
    ...
    #print(X_mean)
    X_std = np.std(unique_X_train, axis=0, dtype=np.float64)+0.0000001 #a small noise
    #print(X_std)
    X_train -= X_mean
    X_train /= X_std
    X_test -= X_mean
...
def main():
    ...
    with tf_graph.as_default():
        ...
        h = nn_base(phs.ximage_flat, phs.keep_prob)
    ...
    for i in range(100): #100 epochs
        ...
        if i % 1 == 0: #always the case
            fd = gp_model.feeds or {}
            fd.update({phs.keep_prob: 1.0, phs.ximage_flat: x_test,
                       phs.label: y_test})
