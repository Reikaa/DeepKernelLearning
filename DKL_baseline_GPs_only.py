...
def main():
    ...
    x_test = dataset[:,0:114]
    y_test = dataset[:,114].reshape(-1,1)
    #print(x_test[20])

    dataset = np.loadtxt("dev", delimiter=",")
    x_valid = dataset[:,0:114]
    y_valid = dataset[:,114].reshape(-1,1)
    #resampleFile()
    dataset = np.loadtxt("train.revised", delimiter=",")
    x_train = dataset[:,0:114]
    y_train = dataset[:,114].reshape(-1,1)
